...
if args.task == "chemprot":
    ...
    print(sklearn.metrics.classification_report(y_pred=pred_class, y_true=test_answer, digits=3))
    results = dict()
    results["f1 score"] = f
    results["recall"] = r
    results["precision"] = p
    
if args.task == "protent":
    pred = [preddf.iloc[i].tolist() for i in preddf.index]
    pred_class = [np.argmax(v) for v in pred]
    str_to_int_mapper = dict()
    ...
    p,r,f,s = sklearn.metrics.precision_recall_fscore_support(y_pred=pred_class, y_true=test_answer, labels=[0,1], average="micro")
    print(sklearn.metrics.classification_report(y_pred=pred_class, y_true=test_answer, digits=3))
    results = dict()
    results["f1 score"] = f
    results["recall"] = r
