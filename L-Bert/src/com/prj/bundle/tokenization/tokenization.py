...
def manage_print(raw_data):
    ...
    if isinstance(raw_data, list):
        string_data ='['
        string_data = string_data+' '.join(i for i in raw_data)
...
class ContextTokenizer(object):
    ''' Runs context based tokenization'''
    
    def __init__(self, kernel_size, stride_size):
        self.contextTokens = {}
        self.kernelSize = kernel_size
        self.strideSize = stride_size
    ...
    def featureConvolution(self, org_Seq, pos_Seq, chunk_Seq, instance):
        ...
        org_sequence = []
        posCluster_sequence = []
        chunkCluster_sequence = []
        startIndex = 0
        endIndex = self.kernelSize
        max_SequenceLength = len(org_Seq)
        while (startIndex < (max_SequenceLength-self.kernelSize+1)) and (endIndex < max_SequenceLength):
            endIndex = (startIndex+self.kernelSize)
            orgBufferList = list(org_Seq[startIndex:endIndex])
            posBufferList = list(pos_Seq[startIndex:endIndex])
            ...
            if(orgBufferList.count('#') == self.kernelSize):
                ...
                '''
                sequence activation
                tf_invar = instance.__semanticInvariance__(posBufferList)
                activation_sequence.append(tf_invar)
                '''
                
                '''
                text-pos invariant calculation
                invar_score = instance.__geometricInvariance__(orgBufferList, posBufferList)
                lead_posCluster_id, complete_posCluster_id = instance.map_to_feature(posBufferList[0], posBufferList, invar_score)
                '''
                ...
                #tf.logging.info("%f"%invar_score+"\t%d"%lead_posCluster_id)
        ...
        return(posCluster_sequence, chunkCluster_sequence, org_sequence, activation_sequence)
        
    def sequencePadding(self, org_Array, pos_Array, chunk_Array, maxTokenLength):
        
        if len(org_Array) < maxTokenLength:
            for index in range(maxTokenLength-len(org_Array)):
                org_Array.append('#')
                pos_Array.append('#')
                chunk_Array.append('#')
        else:
            org_Array = org_Array[0:maxTokenLength]
            pos_Array = pos_Array[0:maxTokenLength]
            chunk_Array = chunk_Array[0:maxTokenLength]
        
        # Santity Check
        if (len(org_Array) != len(pos_Array)) and (len(org_Array) != len(chunk_Array)):
    ...
    def convert_to_context(self, org_Seq, pos_Seq, chunk_Seq, max_seq_length, instance):
        ...
        chunk_Array = list(chunk_Seq.split())
        org_Array, pos_Array, chunk_Array = self.sequencePadding(
            org_Array, pos_Array, chunk_Array, max_seq_length)
        posCluster_sequence, chunkCluster_sequence, org_sequence, activation_sequence = self.featureConvolution(
            org_Array, pos_Array, chunk_Array, instance)
...
class FullTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        split_tokens = []
        wordIndex = 0
        split_array = list(text.split())
        for text in split_array:
            for token in self.basic_tokenizer.tokenize(text):
                for sub_token in self.wordpiece_tokenizer.tokenize(token):
                    split_tokens.append(sub_token)
                    pos_tokens.append(wordIndex)
            wordIndex += 1
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        for token in orig_tokens:
            ...
            '''
            if re.match('^BENTITY$', token):
                split_tokens.append('[unused15]')
            else:
